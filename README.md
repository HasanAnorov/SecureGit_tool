...
#### SYNOPSIS
 ```bash
securegit checkout <plaintext_repo_path> <encrypted_repo_path> [git checkout args...]
```
...
#### EXAMPLES
 ```bash
# Create a new branch branch_name from the current HEAD:
$ securegit checkout plain encrypted -b branch_name
# swift to `branch_name`:
$ securegit checkout plain encrypted branch_name
```

### 14.`securegit merge`- Secure version of `git merge`

#### SYNOPSIS
 ```bash
securegit merge <plain_repo_path> <encrypted_repo_path> <branch_name_a> <branch_name_b>
```
...
#### EXAMPLES
 ```bash
# merge branch_name_b into branch_name_a:
$ securegit merge plain encrypted branch_name_a branch_name_b
```
...
When you run `securegit merge` and both branches modify the same part of a file, Git cannot automatically determine which version to keep. Instead of choosing for you, Git marks the conflict directly inside the file so that you can resolve it manually. For example, 
 ...
 <<<<<<< HEAD
int x = 10;
printf("Current branch version\n");
=======
int x = 20;
printf("Merged branch version\n");
>>>>>>> feature-branch
 ```
